...
fun EditNilaiScreen(
    ...
    Column(
        ...
        when (mataKuliahList) {
            ...
            is Resource.Loading -> {}
            is Resource.Success -> {
                val mataKuliah = mataKuliahList.data?.find { it.id == mataKuliahId }
                if (mataKuliah == null) {
                    return
                }
                ...
                val jumlahSKS =
                    remember { mutableStateOf(mataKuliah.tambahNilai.jumlahSKS.toString()) }
                val tambahKomponen = remember { mutableStateOf(mataKuliah.komponenNilai != null) }
                val nilaiTugas = remember {
                    mutableStateOf(
                        mataKuliah.komponenNilai?.nilaiTugas?.toString() ?: "0"
                    )
                }
                val nilaiKuis = remember {
                    mutableStateOf(
                        mataKuliah.komponenNilai?.nilaiKuis?.toString() ?: "0"
                    )
                }
                val nilaiUTS = remember {
                    mutableStateOf(
                        mataKuliah.komponenNilai?.nilaiUTS?.toString() ?: "0"
                    )
                }
                val nilaiUAS = remember {
                    mutableStateOf(
                        mataKuliah.komponenNilai?.nilaiUAS?.toString() ?: "0"
                    )
                }
                val persentaseTugas = remember {
                    mutableStateOf(
                        mataKuliah.komponenNilai?.persentaseTugas?.toString() ?: "25"
                    )
                }
                val persentaseKuis = remember {
                    mutableStateOf(
                        mataKuliah.komponenNilai?.persentaseKuis?.toString() ?: "25"
                    )
                }
                val persentaseUTS = remember {
                    mutableStateOf(
                        mataKuliah.komponenNilai?.persentaseUTS?.toString() ?: "25"
                    )
                }
                val persentaseUAS = remember {
                    mutableStateOf(
                        mataKuliah.komponenNilai?.persentaseUAS?.toString() ?: "25"
                    )
                }
                ...
                Row(
                    ...
                    Button(onClick = {
                        ...
                        if (nilai.value.toFloat() !in 0.0..4.0) {
                            Toast.makeText(context, "Nilai dalam skala 4", Toast.LENGTH_SHORT)
                                .show()
                            return@Button
                        }
                        if (tambahKomponen.value) {
                            ...
                            if (emptyFields.isNotEmpty()) {
                                ...
                                emptyFields.forEach {
                                    it.first.value = distributedPercentage.toString()
                                }
                            }
                        }
